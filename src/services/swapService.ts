...
export const executeSwap = async (
  ...
  // 4. 最適なPool(Fee Tier)をスキャン & シミュレーション結果(amountOut)取得
  let bestFee = 3000;
  let maxOut = BigInt(0);
  let feeFound = false;
  ...
  // ★修正: トランザクション情報だけでなく、シミュレーションで得た受取額(maxOut)も返す
  // (厳密にはイベントログから拾うべきだが、簡便のためstaticCallの結果を使用)
  return { tx, amountOutRaw: maxOut };
};